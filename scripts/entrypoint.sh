...
# Use standard Postgres env vars if available, fallback to typical defaults
DB_HOST_M="${PGHOST:-${DB_HOST:-postgres}}"
DB_PORT_M="${PGPORT:-${DB_PORT:-5432}}"
DB_USER_M="${PGUSER:-${DB_USER:-postgres}}"

while [ $attempt -le $max_attempts ]; do
  if pg_isready -h "$DB_HOST_M" -p "$DB_PORT_M" -U "$DB_USER_M" 2>/dev/null; then
