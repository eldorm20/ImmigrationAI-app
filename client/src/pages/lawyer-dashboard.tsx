...
export default function LawyerDashboard() {
  ...
  const [compactNav, setCompactNav] = useState<boolean>(() => {
    try {
      return localStorage.getItem('navbarCompact') === '1';
    } catch (_) {
      return false;
    }
  });
                  ...
                  <tr key={lead.id} className="group hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <span className={`px-3 py-1 rounded-full text-xs font-bold border ${
                        lead.status === 'Approved' ? 'bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-800 text-green-600 dark:text-green-400' : 
