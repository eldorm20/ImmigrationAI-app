        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                <AnimatedCard className="bg-gradient-to-br from-green-500 to-emerald-600 text-white border-none shadow-lg shadow-green-500/20">
                    ...
                    <h3 className="text-3xl font-black">{invoices.filter(i => i.status === 'paid').reduce((sum, i) => sum + parseFloat(i.amount), 0).toLocaleString()} UZS</h3>
                </AnimatedCard>
                <AnimatedCard className="bg-white dark:bg-slate-900">
                    ...
                    <h3 className="text-3xl font-black text-slate-900 dark:text-white">{invoices.filter(i => i.status === 'sent' || i.status === 'overdue').reduce((sum, i) => sum + parseFloat(i.amount), 0).toLocaleString()} UZS</h3>
                </AnimatedCard>
                <AnimatedCard className="bg-white dark:bg-slate-900">
                            ...
                            {invoices.length === 0 ? (
                                ...
                                invoices.map(invoice => (
                                    <tr key={invoice.id} className="group hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors">
                                        ...
                                        <td className="px-6 py-4 font-black text-slate-900 dark:text-white">{parseFloat(invoice.amount).toLocaleString()} UZS</td>
                                        <td className="px-6 py-4">
                        ...
                        <div className="pt-6 flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-2xl font-black text-slate-900 dark:text-white">
                                    {newInvoice.items.reduce((sum, item) => sum + (parseFloat(item.amount) || 0), 0).toLocaleString()} UZS
                                </p>
                            </div>
