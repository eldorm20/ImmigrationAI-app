...
export const AIDocsView = ({ applicationId }: { applicationId?: string }) => {
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            let errorMessage = err instanceof Error ? err.message : "Failed to generate document";
            let errorTitle = t.docs.genError;

            // Specific handling for 403 Forbidden
            if (err && typeof err === 'object' && 'statusCode' in err && err.statusCode === 403) {
                errorTitle = "Access Denied";
                errorMessage = "Specialized legal documents can only be generated by licensed immigration lawyers. Please contact a professional for assistance.";
            }
                    ...
                    {mode === 'generate' ? (
                        <div className="space-y-5 mb-8 flex-1 overflow-y-auto pr-3 custom-scrollbar z-10">
                            <div className="grid grid-cols-1 gap-5">
                                {getDocumentFields(docType).map((field: DocumentField) => (
                                    <div key={field.name} className={field.type === 'textarea' ? 'col-span-1' : ''}>
                                        <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1.5 block pl-1">
                                        ...
                                        {field.type === 'textarea' ? (
                                            ...
                                            <select
                                                className="w-full px-5 py-4 rounded-2xl bg-white/50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 text-sm font-bold outline-none focus:ring-4 focus:ring-brand-500/20 focus:border-brand-500 transition-all text-slate-900 dark:text-white appearance-none"
                                                required={field.required}
                                                value={formData[field.name] || ''}
                                                onChange={e => setFormData({ ...formData, [field.name]: e.target.value })}
                                            >
                                ...
                                {getDocumentFields(docType).length === 0 && (
