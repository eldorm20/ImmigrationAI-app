...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    // Permission check: Lawyers and Admins can generate anything.
    // Applicants can generate "submission" and "personal" categories.
    const templateObj = DOCUMENT_TEMPLATES.find(t => t.name === template || t.id === template);
    const isGeneralCategory = templateObj && (templateObj.category === "submission" || templateObj.category === "personal");
    const isClientAllowed = role === "applicant" && isGeneralCategory;

    if (role !== "lawyer" && role !== "admin" && !isClientAllowed) {
      logger.warn({ userId, role, template, category: templateObj?.category }, "Document generation access denied: specialized legal template restricted");
      return res.status(403).json({
        message: "Access denied. Specialized legal documents can only be generated by licensed immigration lawyers. General templates (Motivation, Cover Letter, CV etc.) are available to all users."
      });
    }
