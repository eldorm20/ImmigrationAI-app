...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    // Applicants can only generate specific "general" documents.
    const clientAllowedTemplates = ['Motivation Letter', 'CV enhancement', 'Cover Letter'];
    const isClientAllowed = role === "applicant" && clientAllowedTemplates.includes(template);

    if (role !== "lawyer" && role !== "admin" && !isClientAllowed) {
      return res.status(403).json({
        message: "Access denied. Specialized legal documents can only be generated by licensed immigration lawyers."
      });
    }
