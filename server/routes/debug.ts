...
router.get("/errors", async (req, res) => {
  ...
  try {
    ...
    if (!isDbUp) {
      errors.issues.push("❌ Database connection test failed.");
    } else {
      errors.issues.push("✅ Database connection test successful.");
    }
  } catch (err) {
    errors.issues.push(`❌ Database connection test errored: ${String(err)}`);
  }
