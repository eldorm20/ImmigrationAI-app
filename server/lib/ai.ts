...
${new Date().toLocaleDateString()}`;
      ...
      if (template === 'Sponsorship Letter') {
        return `CERTIFICATE OF SPONSORSHIP / SUPPORT LETTER

To: UK Visas and Immigration

RE: Sponsorship of ${data.name || '[Applicant Name]'}
Role: ${data.role || '[Job Title]'}
Salary: [Salary Amount]

Dear Sir/Madam,

We, ${data.company || '[Company Name]'}, confirm that we are sponsoring ${data.name || '[Applicant Name]'} for a Skilled Worker Visa.

DETAILS OF EMPLOYMENT
The applicant will be employed as a ${data.role || '[Job Title]'} (SOC Code: [Code]). Their key duties will include:
${data.skills || '• [Duty 1]\n• [Duty 2]'}

SUITABILITY
The applicant is highly suitable for this role due to their background in ${data.education || '[Education]'} and experience of ${data.experience || '[Years]'} years.

MAINTENANCE
We certify that we will maintain and accommodate the applicant for the first month of their employment if necessary.
...
${data.company || '[Company Name]'}`;
      ...
      if (template === 'Family Visa Letter') {
        return `LETTER OF SUPPORT FOR FAMILY VISA

To: The Entry Clearance Officer

RE: Application of ${data.name || '[Applicant Name]'} (Spouse/Partner/Child)
Sponsor: [Sponsor Name]

Dear Sir/Madam,

I am writing to confirm my sponsorship of the visa application for my ${data.role || 'partner'}, ${data.name || '[Applicant Name]'}.

RELATIONSHIP
We have been in a genuine and subsisting relationship since [Date]. We intend to live together permanently in the UK at [Address].

FINANCIAL REQUIREMENT
I am currently employed at ${data.company || '[Company]'} earning [Salary], which meets the financial requirement. Envelope enclosed includes my payslips and bank statements.

ACCOMMODATION
We will reside at [Address], which is owned/rented by me and has sufficient space for us without overcrowding.

I respectfully request that you grant this visa.
