...
export async function uploadFile(
  ...
  if (process.env.DATABASE_URL && !BUCKET_NAME) {
    try {
      // Use the shared pool from ../db to avoid connection exhaustion
      ...
      // Execute with the shared pool
      // Note: We use pool.query directly. 
      // We do NOT call pool.connect() + release() manually because pool.query handles checkout/release automatically for single queries.
      // This prevents leaking clients if errors occur.

      await pool.query(createSql);
      await pool.query(
        'INSERT INTO file_blobs(key, file_data, file_name, mime_type, file_size) VALUES($1,$2,$3,$4,$5) ON CONFLICT (key) DO NOTHING',
        [key, file.buffer, file.originalname, file.mimetype, file.size]
      );

      // Use a relative URL for the blob endpoint
      const url = `/api/documents/blob/${encodeURIComponent(key)}`;
      logger.info({ key, userId, method: 'database' }, 'Stored file in Postgres (persistent)');
      ...
      // If DB storage fails, this is critical. 
      // Do NOT fallback to FS because it is ephemeral. 
      // Throw error to alert user rather than silently losing data later.
      logger.error({ err, key, userId }, "CRITICAL: Database blob storage failed");
      throw new Error("Failed to store file in database. Please contact support.");
    }
  }

  // If BUCKET_NAME is not configured and NO DATABASE_URL (unlikely), use local filesystem storage
  ...
  try {
    ...
    if (process.env.DATABASE_URL) {
      try {
        ...
        await pool.query(createSql);
        await pool.query('INSERT INTO file_blobs(key, file_data, file_name, mime_type, file_size) VALUES($1,$2,$3,$4,$5) ON CONFLICT (key) DO NOTHING', [key, file.buffer, file.originalname, file.mimetype, file.size]);
    ...
    try {
      ...
      fs.writeFileSync(destPath, file.buffer);
      const url = `${process.env.APP_URL || "/"}/uploads/${key}`;
      logger.info({ key, userId }, 'Stored file on local filesystem as fallback after S3 failure (WARNING: EPHEMERAL)');
      return {
        key,
        url,
        fileName: file.originalname,
